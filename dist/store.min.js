...
return altItems || (lastParams = strParams, lastQueriedItems = items), items;
},
load: function(url) {}
...
i = a + 1; i <= b; i++) selected.push(dataStore.getItemByIndex(i)); else selected = [ item ];
}
} else selected = item;
lastSelected = item;
}
...
get: function() {
return selected ? Array.isArray(selected) ? dataStore.query(0, selected) : [ selected ] : null;
},
