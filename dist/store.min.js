...
set: function(items) {
this.orgItems = items, this.items = items.concat([]);
},
...
for (lastParams = strParams, items = this.items.concat([]), i = 0; i < plugins.length; i++) items = plugins[i](items, currentParams, this);
return items;
},
load: function(url) {}
};
return toArray(options.plugins).forEach(function(pluginName) {
var i, plugin = store.plugins[pluginName], order = plugin.order;
if (plugins.length) if (1 === plugins.length) plugins[0].order <= order ? plugins.push(plugin) : plugins.unshift(plugin); else {
...
}, "function" == typeof customLoader) customLoader(store, "store"); else if ("undefined" != typeof window) window.store = store; else {
if ("undefined" == typeof module) return store;
