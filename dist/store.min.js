...
options = options || {};
var dataStore, defaults = {
identifier: "id"
}, plugins = [], lastParams = "", currentParams = {};
...
return this.items ? void (Array.isArray(itemOrItems) ? this.items = this.items.concat(itemOrItems) : this.items.push(itemOrItems)) : this.set(itemOrItems);
},
...
items && !altItems && strParams === lastParams) return items;
for (altItems || (lastParams = strParams), i = 0; i < plugins.length; i++) items = plugins[i](items, currentParams, this);
return items;
},
load: function(url) {}
...
if (!multiple) return item === selected;
for (var i = 0; i < selected.length; i++) if (selected[i] === item) return !0;
return !1;
}
function select(item) {
multiple ? Array.isArray(selected) ? selected.indexOf(item) === -1 && selected.push(item) : selected = [ item ] : selected = item;
}
function unselect(item) {
multiple ? selected = selected.filter(function(m) {
return m !== item;
}) : item === selected && (selected = null);
}
var selected, opts = dataStore.options.selection || {}, multiple = !!opts.multiple, additive = opts.additive;
...
get: function() {
return multiple ? dataStore.query(0, selected) : selected;
},
...
Array.isArray(itemOrId) ? multiple ? (additive || (selected = null), itemOrId.forEach(setter)) : console.error("To make a multi-selection, use `store({plugins: ['selection'], multiple:true})`") : setter(itemOrId);
}
});
}
...
if (store.addPlugin("sort", sort, 20), "function" == typeof customLoader) customLoader(store, "store"); else {
